...
export class ProfessorProfile extends Component {
  ...
  loadUser = () => {
    ...
    ref.where("uid", "==", this.props.navigation.state.params.uid).get().then(function(querySnapshot) {
        var user = {}
  ...
  loadClasses = () => {    
    ...
    let array = []
    ref.where("professorUid", "==", this.state.user.uid).get().then(function(querySnapshot) {
        querySnapshot.forEach(function(doc) {
            array.push(doc.data());
        })
